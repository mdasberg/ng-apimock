(function () {
    ...
    function ngApiMockRequest(request, response, next) {
        var ngapimockId = ngApimockId(request.headers);
        if(ngapimockId !== undefined) {
            // if either a ngApimockId cookie or header has been found set the header.
            request.headers.ngapimockid = ngapimockId;
        }
